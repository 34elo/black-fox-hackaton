...
@router_admin_panel.message(F.text == "Отправить уведомление сотрудникам")
...
async def smena(message: Message, state: FSMContext) -> None:
    try:
        ...
        await message.answer('Напишите ФИО сотрудника, которого вы хотите назначить на смену')
...
async def worker(message: Message, state: FSMContext) -> None:
    try:
        ...
        await message.answer('График сформирован. Ознакомиться с ним вы можете в меню "Расписание на точках"')
        await state.clear()
    except Exception as e:
        print(e)
        await message.answer('Ошибка, попробуйте снова')
...
@router_admin_panel.message(F.text == 'Количество смен у сотрудников')
async def top_users(message: Message, state: FSMContext) -> None:
    text = 'Количество смен у сотрудников\n\n'
